...
socket.element.addEventListener("projectiles", event => {
	...
	for (let projectile of projectiles) {
		...
		if (!projectileElement) {
			...
			// Image resources only have T and CT variants; for unknown team "U",
			// fall back to T image but mark with "U" class so CSS can render it neutral.
			let imgTeam = (projectile.team === "CT" || projectile.team === "T") ? projectile.team : "T"
			...
			// Color it with the team colors; gray for unknown team
			let trailColor = projectile.team == "T" ? "#ffa70166" : projectile.team == "CT" ? "#1584C466" : "#88888866"
			trailElement.setAttributeNS(null, "stroke", trailColor)
