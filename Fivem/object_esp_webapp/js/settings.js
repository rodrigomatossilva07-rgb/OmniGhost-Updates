/* settings.js - FiveM Object ESP Settings Management */
...
const SettingsManager = {
	...
	applySettings() {
		...
		if (this.elements.languageSelect) {
			this.elements.languageSelect.value = localStorage.getItem('object_esp_lang') || 'pt';
		}
	...
	applySingleSetting(key, value) {
		switch (key) {
			case 'theme':
				...
				break;
			case 'followRotation':
				if (window.ObjectESPApp) window.ObjectESPApp.state.followRotation = value;
				break;
			case 'autoZoom':
				if (window.ObjectESPApp) window.ObjectESPApp.state.autoZoom = value;
				break;
			case 'minZoom':
				if (window.ObjectESPApp && window.ObjectESPApp.state.zoom < value) {
					window.ObjectESPApp.setZoom(value);
				}
				break;
			case 'mapStyle':
				if (window.ObjectESPApp) window.ObjectESPApp.updateMapStyle();
				break;
			case 'playerDotScale':
				if (window.ObjectESPApp) window.ObjectESPApp.updateAllMarkerSizes();
				break;
			case 'boxThickness':
				if (window.ObjectESPApp) window.ObjectESPApp.renderer.SetBoxThickness(value);
				break;
		}
	},
	...
	exportSettings() {
		...
		a.href = url;
		a.download = `object-esp-settings-${Date.now()}.json`;
		a.click();
