/* map.js - FiveM Radar Map Rendering & Interactions */

const MapRenderer = {
	// Map calibration constants for GTA V world coordinates
	// GTA V world: X = East/West, Y = North/South, Z = Up/Down
	// Map image coordinates: 0,0 = top-left
	
	CALIBRATION: {
		// Los Santos main map (satellite view)
		los_santos: {
			// World bounds covered by the map image
			worldBounds: {
				minX: -4000, maxX: 4000,
				minY: -4000, maxY: 8000
			},
			...
			// Origin offset (where world 0,0 maps to image pixels)
			origin: { x: 2048, y: 2048 },
			// Scale: world units per pixel
			scale: 1.953 // ~2 world units per pixel
		},
	...
	// Convert world coordinates to map image pixels
	worldToMap(worldPos, mapType = 'los_santos') {
		...
		return {
			...
			y: cal.origin.y - worldPos.y / cal.scale // Y inverted
		};
	},
	
	// Convert map image pixels to world coordinates
	mapToWorld(mapPos, mapType = 'los_santos') {
		const cal = this.CALIBRATION[mapType] || this.CALIBRATION.los_santos;
		return {
			x: (mapPos.x - cal.origin.x) * cal.scale,
			y: (cal.origin.y - mapPos.y) * cal.scale // Y inverted
		};
	},
	
	// Convert world coordinates to radar screen coordinates (for markers)
	worldToRadar(worldPos, radarState, mapType = 'los_santos') {
		const cal = this.CALIBRATION[mapType] || this.CALIBRATION.los_santos;
		const mapPos = this.worldToMap(worldPos, mapType);
		
		// Apply radar transform (zoom, pan, rotation)
	...
	// Get current map type based on player position
	...
	drawGraticule(ctx, radarState, mapType = 'los_santos') {
		...
		const gridSize = 500 * zoom; // 500 world units
	...
	drawCoordinates(ctx, radarState, mapType = 'los_santos') {
		...
		// Mouse world position (if tracking)
