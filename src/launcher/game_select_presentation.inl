...
void DrawHeader(ImDrawList* draw, ImVec2 display) {
    ...
    const std::string masked = MaskEmail(auth.CurrentEmail());
    ...
    char initial[2] = {'O', 0};
    if (!auth.CurrentEmail().empty())
        initial[0] = static_cast<char>(std::toupper(static_cast<unsigned char>(auth.CurrentEmail()[0])));
...
void ActivateGame(GameRuntime& runtime) {
    ...
    if (IsReadyState(runtime.state) || attach_to_running) {
        ...
        // Do not probe/open FPGA from the launcher. Only block immediately when
        // the local DMA runtime itself is invalid; physical device availability is
        // checked by the explicit game launch path.
        if (!mem.GetDiagnosticsSnapshot().dependencyIntegrityOk) {
            runtime.state = CardState::DeviceMissing;
            g_help_game = game.launch_id;
            return;
        }
...
void DrawCardContextMenu(const GameDefinition& game, ImVec2 anchor) {
    ...
    if (!fg) return;
    const float width = S(184.f);
    const float row = S(34.f);
    const int count = 5;
    const ImVec2 menuMin(anchor.x - width, anchor.y);
    const ImVec2 menuMax(anchor.x, anchor.y + row * count + S(12.f));
    fg->AddRectFilled(ImVec2(menuMin.x, menuMin.y + S(4.f)), ImVec2(menuMax.x, menuMax.y + S(5.f)),
        CyberTheme::SafeShadowU32(85), CyberTheme::Radius::Md);
    fg->AddRectFilled(menuMin, menuMax, C_PANEL(252), CyberTheme::Radius::Md);
    fg->AddRect(menuMin, menuMax, CyberTheme::WithAlpha(CyberTheme::Colors.Border, 0.58f), CyberTheme::Radius::Md);
    ...
    const MenuAction actions[] = {
        ...
        {history.favorite ? Loc::Tr("launcher.action.unpin") : Loc::Tr("launcher.action.pin"), 3},
        {Loc::Tr("launcher.action.open_logs"), 4},
    };
    float y = menuMin.y + S(6.f);
    bool anyHovered = false;
    for (const MenuAction& item : actions) {
        const ImVec2 itemMin(menuMin.x + S(6.f), y);
        const ImVec2 itemMax(menuMax.x - S(6.f), y + row);
        const bool hovered = ImGui::IsMouseHoveringRect(itemMin, itemMax, false);
        anyHovered = anyHovered || hovered;
        if (hovered) {
            ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
            fg->AddRectFilled(itemMin, itemMax, C_CARD_TOP(), CyberTheme::Radius::Sm);
        }
        fg->AddText(ImVec2(itemMin.x + S(10.f), itemMin.y + S(8.f)), hovered ? C_GOLD_LT() : C_TEXT(), item.label);
        if (hovered && ImGui::IsMouseClicked(0)) {
    ...
    if (ImGui::GetFrameCount() != g_card_menu_open_frame && ImGui::IsMouseClicked(0) &&
        !anyHovered && !ImGui::IsMouseHoveringRect(menuMin, menuMax, false))
        g_card_menu_game = GameId::None;
}
...
bool DrawGameCard(ImDrawList* draw, ImVec2 min, ImVec2 max,
    ...
    if (moreHovered && ImGui::IsMouseClicked(0)) {
        g_card_menu_game = g_card_menu_game == game.launch_id ? GameId::None : game.launch_id;
        g_card_menu_open_frame = ImGui::GetFrameCount();
    }
    ...
    else if (runtime.state == CardState::NotInstalled) action = Loc::Tr("launcher.action.how_fix");
