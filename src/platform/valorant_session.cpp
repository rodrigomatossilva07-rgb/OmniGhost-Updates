...
namespace {
class ValorantSession final : public IGameSession {
public:
    explicit ValorantSession(const ServiceContainer&) : state_(SessionState::Detached) {}
    ...
    [[nodiscard]] std::string_view GetStatus() const noexcept override { return "Valorant"; }
...
std::unique_ptr<IGameSession> CreateValorantSession(const ServiceContainer& services) {
