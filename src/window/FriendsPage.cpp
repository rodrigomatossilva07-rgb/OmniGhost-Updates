...
void DrawFriends()
{
    ...
    CyberWidgets::BeginCard(Loc::Tr("fr.list"), half);
    CyberWidgets::TextLineF(CyberWidgets::TextTone::Secondary,
        Loc::Tr("fr.nearby"), (int)friends::player_list.size());
    CyberWidgets::Separator();
    CyberWidgets::ToggleSwitch(Loc::TrID("fr.ignore_aim"), &friends::config.ignore_aim);
    CyberWidgets::ToggleSwitch(Loc::TrID("fr.ignore_silent"), &friends::config.ignore_silent);
    CyberWidgets::ToggleSwitch(Loc::TrID("fr.show_esp"), &friends::config.show_friend_esp);
    CyberWidgets::ColorEditU32(Loc::TrID("fr.color"), &friends::config.friend_color);
    CyberWidgets::ToggleSwitch(Loc::TrID("fr.add_prox"), &friends::config.add_by_prox);
    CyberWidgets::SliderFloat(Loc::TrID("fr.prox_dist"), &friends::config.prox_max_distance, 5.0f, 200.0f, "%.0f m");
    if (CyberWidgets::GoldButton(Loc::TrID("fr.add_closest"), ImVec2(150, 34))) {
        friends::AddClosestAsFriend();
        CyberWidgets::Notify(Loc::Tr("status.friend_added"), CyberWidgets::ToastType::Success);
    }
    ImGui::SameLine();
    if (CyberWidgets::CyberButton(Loc::TrID("fr.remove_closest"), ImVec2(150, 34)))
        friends::KickClosestFromFriends();
    if (CyberWidgets::CyberButton("Export Amigos", ImVec2(150, 34))) {
        if (friends::ExportFriendsClipboard())
            CyberWidgets::Notify(Loc::Tr("status.copied"), CyberWidgets::ToastType::Success);
    }
    ImGui::SameLine();
    if (CyberWidgets::CyberButton("Import Amigos", ImVec2(150, 34))) {
    ...
    CyberWidgets::BeginCard(Loc::Tr("fr.players"), half);
